...
include("ayar.php");
	...
	$kime=$_POST['kime'];
		if($_POST['gonder']){	
		mysql_query("insert into umesaj values (NULL,'".$konu."','".$mesaj."','".$_GET["id"]."','".$kime."',NOW(),'1')")or die(mysql_error());	
		echo "Mesajınız İletildi.";
		}
		if($_GET["del"]){
mysql_query("delete from umesaj where id=$_GET[del]")or die(mysql_error());	
echo "Mesaj Silindi";
}
?>
<style type="text/css">
...
<?php
$s = mysql_query("select * from uyelik where id<>'".$_GET["id"]."'");
while ($lis = mysql_fetch_array($s)) {
?>
      <option value="<?=$lis["id"];?>"><?=$lis["adi_soyadi"];?> - <?=$lis["numara"];?></option>
...
<tr>
  ...
  <th width="17" scope="col"></th>
    <th width="124" scope="col">Kimden</th>
    <th width="124" scope="col">Konu</th>
    <th width="264" scope="col">Mesaj</th>
    <th width="126" scope="col">tarih</th>
    <th width="31" scope="col">Sil</th>
    </tr>
  
  <?php
  $i=0;
  $s = mysql_query("select * from umesaj where kime='".$_GET["id"]."' order by id");
while ($lis = mysql_fetch_array($s)) {
	$list = mysql_fetch_assoc(mysql_query("select * from uyelik where id=".$lis["kimden"].""));
	$i++;
  ...
  <tr>
    ...
    <td><?=$lis["mesaj"];?></td>
    <td><?=$lis["tarih"];?></td>
    <td><a href="uindex.php?s=umesaj&id=<?=$_GET["id"];?>&del=<?=$lis["id"];?>">Sil</a></td>
    </tr>
    <?php
  }
   ?>
